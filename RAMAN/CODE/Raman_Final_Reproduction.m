...
%% RAMAN_FINAL_REPRODUCTION
% Reconstruct and test the final Raman workflow using the ORIGINAL split.
%
...
%
% Optional:
%   VarLabels (rows 4:407 contain the 404 Raman wavenumbers)
%
% The script:
...
%% -------------------- SETTINGS --------------------
outDir = fullfile(pwd,'Raman_Final_Outputs');
...
target.Raw.PLS = [0.4895, 0.8378];
target.Raw.SVR = [0.5290, 0.8106];
target.MSC.PLS = [0.5255, 0.8131];
target.MSC.SVR = [0.6306, 0.7308];
target.SNV.PLS = [0.5251, 0.8133];
target.SNV.SVR = [0.7288, 0.6404];
target.SG1.PLS = [0.7395, 0.6702];
target.SG1.SVR = [1.2413, 0.0707];
target.SG2.PLS = [0.7923, 0.6208];
target.SG2.SVR = [1.3049, -0.0269];
...
assert(size(X,2)==3401,'Expected Raman X to contain 3401 spectral variables.');
...
if exist('VarLabels','var')
    try
        wavenumber = str2double(strtrim(cellstr(VarLabels(3:3403,:))));
...
if ~isempty(wavenumber)
    makeRamanFigureSet(Xprep,wavenumber,outDir);
end
...
for p = 1:numel(prepNames)
    ...
    save(fullfile(outDir,sprintf('Raman_%s_PLSR_Model.mat',prep)), ...
    ...
    % Raman SVR: automatic/default hyperparameters.
    if strcmp(prep,'SG1') || strcmp(prep,'SG2')
        SVRModel = fitrsvm(Xtr,Ytrain,'KernelFunction','gaussian', ...
            'Standardize',true,'KernelScale',50);
    else
        SVRModel = fitrsvm(Xtr,Ytrain,'KernelFunction','gaussian', ...
    ...
    save(fullfile(outDir,sprintf('Raman_%s_SVR_Model.mat',prep)), ...
...
writetable(Results,fullfile(outDir,'Raman_Model_Performance_Reproduction.csv'));

save(fullfile(outDir,'Raman_Final_Model_Bundle.mat'), ...
...
Selected_Raman_SNV_SVR = Models.SNV.SVR;
Selected_Raman_SG1_PLS  = Models.SG1.PLS;
save(fullfile(outDir,'Raman_Selected_Models.mat'), ...
    'Selected_Raman_SNV_SVR','Selected_Raman_SG1_PLS');

fprintf('\nRaman reproduction complete. Outputs saved to:\n%s\n',outDir);
...
function makeRamanFigureSet(Xp,wv,outDir)
    names=fieldnames(Xp);
    for i=1:numel(names)
        prep=names{i};
        ...
        title(['Raman ',prep,' Spectra']);
        grid on; box on; xlim([min(wv) max(wv)]);
        exportgraphics(f,fullfile(outDir,['Raman_',prep,'_Spectra.png']),'Resolution',300);
        close(f);
    end
end
